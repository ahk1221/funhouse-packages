...
jcmp.events.Add('Render', renderer => {
    if(shouldShowBanScreen) {
        ...
        drawText(renderer, banText, new Vector3f(renderer.viewportSize.x / 2, renderer.viewportSize.y / 9.14, 0), 45, "Impact");

        drawText(renderer, "BAN REASON:", new Vector3f(renderer.viewportSize.x / 2, renderer.viewportSize.y / 5.2, 0), 45, "Impact");

        drawText(renderer, reasonText.toUpperCase(), new Vector3f(renderer.viewportSize.x / 2, renderer.viewportSize.y / 3.5, 0), 45, "Impact");

        if(timeTillUnban !== undefined)
            drawText(renderer, timeTillUnban, new Vector3f(renderer.viewportSize.x / 2, renderer.viewportSize.y / 2, 0), 45, "Impact");
    }
});
